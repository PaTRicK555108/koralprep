//document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleSongChange(e) {
      if (mmaSelect.value == 'null') mmaSelect.value = "mi";
    ...
    function updatePlayerUI() {
        ...
        timeSlider.value = progress;//miaudio.currentTime;//progress;
    ...
    function updateDurationDisplay() {
        ...
        //timeSlider.max = miaudio.duration;
    }
    ...
    async function loadLyrics(url) {
        try {
            ...
            text.split('\n').forEach(line => {
                ...
                // lineElement.addEventListener('click', () => seekToLyric(seconds) );
...
function highlightCurrentLyrics() {
    ...
    // جلب كل التواقيت الموجودة في الأسطر (lines) وتحويلها لأرقام
    const times = lines.map(line => parseFloat(line.dataset.time));
    
    // البحث عن أقرب توقيت أقل من أو يساوي currentTime
    ...
    // إزالة هايلايت من كل الأسطر
    lines.forEach(line => line.classList.remove('highlight'));
    // إضافة هايلايت للسطر الذي يطابق أقرب توقيت
    if (closestTime !== null) {
        lines.forEach(line => {
            if (parseFloat(line.dataset.time) === closestTime) {
                line.classList.add('highlight');
      if (!isUserScrolling) isProgrammaticScroll = true && line.scrollIntoView({ behavior: 'smooth', block: 'center' });
               setTimeout(() => {
    ...
    function handleLyricClickForSolo(time) {
        if (solo.status === 'selecting-start') {
            ...
            //alert('الآن الرجاء النقر على السطر الذي يحدد نهاية الصولو');
